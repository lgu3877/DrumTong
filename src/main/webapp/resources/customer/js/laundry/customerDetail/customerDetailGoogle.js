...
function displayChart() {
	...
	for (i = 0; i < reviewList.length; i++) { // 구글 차트 그래프에 점수 넣는 기능
		data.setCell((reviewList[i].gpa * 2) - 1, 1, data.getValue(
				(reviewList[i].gpa * 2) - 1, 1) + 1);
		data.setCell((reviewList[i].gpa * 2) - 1, 2, data.getValue(
				(reviewList[i].gpa * 2) - 1, 2) - 1);
	}
...
function setScore() {
	...
	let cscore = checkReviewExist ? (avgcscore / reviewList.length).toFixed(2)
			: '없음';
