  	...
  	<script>
		function pwChange(bpersonid){
			...
			if(!regExp.test(password)){
				alert('최소 7자리에서 15자리까지의 영문, 숫자, 특수문자 조합으로 구성해주세요');
				return false;
			}
		      var axPost = async () => {
		        ob={
...
<body>
	...
	<section>
		<div class="pw_reset_con">
			<form class="pw_reset_view" method="POST" action="${cpath }/business/membership/${AccountPage == 'AccountPage' ? 'businessAccountInfo/' : 'businessPwChange/'}" onsubmit="return pwChange('${User.bpersonid}');">
